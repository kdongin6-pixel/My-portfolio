...
| 파일 | 역할 |
|------|------|
| `index.html` | 앱 전체 — HTML + CSS + JS 단일 파일 (~2,400줄). GitHub Pages로 배포 |
...
배포: main 브랜치 push → GitHub Pages 자동 배포 (kdongin6-pixel.github.io/My-portfolio)
...
- 바닐라 JS, 프레임워크·빌드 없음. 압축된 스타일 (한 줄 CSS, 짧은 변수명)
...
- 거래 수수료·세금 미반영 → 앱 현금이 실제보다 많아짐 (수수료율 설정 기능 논의 중)
...
- 스크린샷 업로드 → Claude API 비전으로 거래내역 자동 반영 기능 논의 중 (API 키 위치: 브라우저 vs GAS 미정)
